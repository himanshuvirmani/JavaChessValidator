...
public class AnMoveInterpreterImpl implements AnMoveInterpreter {

    private ChessRules chessRules;

    public AnMoveInterpreterImpl() {
    }

    public AnMoveInterpreterImpl(ChessRules chessRules) {
        this.chessRules = chessRules;
    }

    @Override
    public String convertMoveToAn(ChessPosition position, ChessMovePath move) throws
    ...
    private String dumpStandardMove(ChessPosition position, ChessMovePath move, String checkMark) throws
        ...
        System.out.println("Creating PGN notation for move " + move);
        System.out.println("Possible sources are " + samePieces);
    ...
    @Override
    public ChessMovePath convertAnToMove(ChessPosition position, String pgnMove) throws
            IllegalMoveException {
        ChessMovePath path;
    ...
    protected ChessMovePath parseStandardMove(String pgnMove, ChessPosition position) throws
        ...
        if (!pgnMatcher.matches()) {
            throw new IllegalMoveException(pgnMove, "Does not match PGN syntax");
        }
        ...
        if (selectedSources.isEmpty()) {
            ...
            throw new IllegalMoveException(pgnMove, "No piece can reach square " + pgnDestination);
        } else if (selectedSources.size() > 1) {
            throw new IllegalMoveException(pgnMove, "Several pieces can reach square " + pgnDestination);
        }

        ChessPieceType promoted = ChessPieceType.QUEEN;
        if (StringUtils.isNotEmpty(pgnPromotion)) {
            System.out.println("PGN promotion is {}" + pgnPromotion);
